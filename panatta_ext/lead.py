...
class crm_lead(osv.osv):
    ...
    def create(self, cr, uid, vals, context=None):
        ...
        ir_config_ids = ir_config_parameter.search(cr,uid,[('key','=','web.base.url')])
        host = ir_config_parameter.browse(cr,uid,ir_config_ids[0]).value
        logger.error('ir_config_ids --- host----  ==== %s', ir_config_ids) 
        logger.error('host --- host----  ==== %s', host) 
        if data_opportunity.user_id.login:
            ...
            url = str((host+'/web?db=panatta_db#id='+str(data_opportunity.id)+'&view_type=form&model=crm.lead&menu_id=138&action=145').encode('utf-8'))
