...
class res_partner(osv.osv):
    ...
    def goto_website(self, cr, uid, ids, context=None):
        partner_obj = self.pool.get('res.partner')
        partner = partner_obj.browse(cr, uid, ids, context=context)[0]
        
        if partner.website:
            good_starting_urls = ['https://', 'http://']
            non_protocol_starting_urls = ['/', '']
            
            if any(map(lambda x: partner.website.startswith(x), good_starting_urls)):
                url = partner.twitter
            elif any(map(lambda x: partner.website.startswith(x), non_protocol_starting_urls)):
                url = 'http://' + partner.website
            else:
                url = partner.website
